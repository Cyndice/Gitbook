...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tipBox = document.getElementById('tutorial-tip-box');
    const understandBtn = document.getElementById('tutorial-understand-btn');
    const navbarButton = document.getElementById('anchor-navigation-ex-navbar'); // 你的目录按钮ID

    // 如果目录按钮不存在，则不执行任何操作
    if (!navbarButton) {
        console.warn("未找到 ID 为 'anchor-navigation-ex-navbar' 的元素，教程提示功能将不启用。");
        return;
    }

    // 检查是否首次访问（LocalStorage）
    ...
    if (!hasVisited) {
        ...
        navbarButton.classList.add('highlight-navbar'); // 给目录按钮添加高亮

        // 定位提示框（简单居中处理，或根据需要调整）
        positionTipBox();
    ...
    understandBtn.addEventListener('click', () => {
        ...
        navbarButton.classList.remove('highlight-navbar'); // 移除高亮
        localStorage.setItem('hasVisitedTutorial', 'true'); // 设置已访问标志

        window.removeEventListener('resize', positionTipBox); // 移除监听器
    });
    ...
    function positionTipBox() {
        ...
        const tipBoxHeight = tipBox.offsetHeight;

        // 尝试放在目录按钮的下方偏左侧
        let tipBoxTop = navbarRect.top + navbarRect.height + 0; // 按钮下方20px
        let tipBoxLeft = navbarRect.left - tipBoxWidth - 15; // 按钮左侧30px

        // 边界检查：确保提示框不会超出屏幕边界
        if (tipBoxLeft < 10) {
            tipBoxLeft = navbarRect.right + 30; // 如果左边放不下，移到右边
            if (tipBoxLeft + tipBoxWidth > window.innerWidth - 10) {
                // 如果右边也放不下，则居中显示
                tipBoxLeft = (window.innerWidth - tipBoxWidth) / 2;
                tipBoxTop = (window.innerHeight - tipBoxHeight) / 2;
            }
        }
        if (tipBoxTop < 10) {
            tipBoxTop = 10;
        }
        if (tipBoxTop + tipBoxHeight > window.innerHeight - 10) {
            if (navbarRect.top - tipBoxHeight - 20 > 10) {
                tipBoxTop = navbarRect.top - tipBoxHeight - 20; // 如果下方放不下，尝试放上方
            } else {
                tipBoxTop = (window.innerHeight - tipBoxHeight) / 2; // 上下都放不下则居中
                tipBoxLeft = (window.innerWidth - tipBoxWidth) / 2;
            }
        }
        
        tipBox.style.top = `${tipBoxTop}px`;
        tipBox.style.left = `${tipBoxLeft}px`;
    }
});